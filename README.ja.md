...
- **アイドル時** - 非アクティブ後に休憩や別のアプローチを促すリマインダー
...
```json
{
  "codeMantra.rules": [
    ...
    {
      "trigger": "onIdle",
      "message": "しばらくコーディングしています - 休憩を取るか別のアプローチを試してみてください！",
      "idleDuration": 15
    }
  ]
}
...
| `idleDuration`      | `onIdle` 用：通知を表示するまでの非アクティブ時間（分、デフォルト: 15）         | 範囲: 1-120 分                                |

**注意:** 複数の `onIdle` ルールが定義されている場合、最初にマッチしたルールのみが発火します。複数のアイドル通知を受け取るには、適切な間隔で異なる `idleDuration` 値を使用してください。
